...
import com.example.runners.databinding.ItemAdapaterBinding
import com.example.runners.model.Task
...
class TaskAdapter(
    ...
    override fun onBindViewHolder(holder: TaskAdapter.MyViewHolder, position: Int) {
        ...
        when (task.status) {
            ...
            1 -> {
                ...
                holder.binding.ibBack.setOnClickListener { taskSelected(task, SLECT_BACK) }
                holder.binding.ibNext.setOnClickListener { taskSelected(task, SELECT_NEXT) }
            }
            else -> {
    ...
    inner class MyViewHolder(val binding: ItemAdapaterBinding) :
